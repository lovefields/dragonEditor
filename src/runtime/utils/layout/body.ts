...
import { _contentKeydownEvent, _contentPasteEvent } from "../event/keyboard";
import { _contentMouseupEvent, _contentMousedownEvnet, _contentTouchstartEvent } from "../event/mouse";
...
export function _getBodyVNodeStructure(store: Ref<DragonEditorStore>): VNode {
    return h(
        ...
        _createBlockList(store.value.firstData)
    );
}

// 블럭 구조체 생성
function _createBlockList(blockList: DEContentData): VNode[] {
    const blockArray: VNode[] = [];

    if (blockList.length === 0) {
        blockArray.push(h("p", { class: ["de-block", "de-text-block"], contenteditable: true }));
    } else {
